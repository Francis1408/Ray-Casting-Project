...
int main() {
    ...
    Shader ourShader("Shaders/shaderCoordinateSystem.vs", "Shaders/shaderTexture.fs");
    ...
    view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f)); // Translate the view matrix by -3 units in the z-axis
    ...
    while (!glfwWindowShouldClose(window)) { // Check if the window should close
        ...
        int modelLoc = glGetUniformLocation(ourShader.ID, "model"); // Get the uniform location of the model matrix
        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model)); // Set the value of the model matrix uniform
        int viewLoc = glGetUniformLocation(ourShader.ID, "view"); // Get the uniform location of the view matrix
        glUniformMatrix4fv(viewLoc, 1, GL_FALSE, glm::value_ptr(view)); // Set the value of the view matrix uniform
        int projectionLoc = glGetUniformLocation(ourShader.ID, "projection"); // Get the uniform location of the projection matrix
        glUniformMatrix4fv(projectionLoc, 1, GL_FALSE, glm::value_ptr(projection)); // Set the value of the projection matrix uniform
