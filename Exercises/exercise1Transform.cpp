...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) { // Check if the window should close
        ...
        // Transform and draw the second rectangle

        glm::mat4 trans2 = glm::mat4(1.0f); // Create the identity matrix
        trans2 = glm::translate(trans2, glm::vec3(-0.5f, 0.5f, 0.0f)); // Apply a translation of -0.5 units in the x and 0.5 units in the y
        float scaleAmount = sin(glfwGetTime()); // Change the scale value over time
        trans2 = glm::scale(trans2, glm::vec3(scaleAmount, scaleAmount, scaleAmount)); // Apply a scale transformation

        // Apply the transformations to the shader
        unsigned int transformLoc2 = glGetUniformLocation(ourShader.ID, "transform"); // Locate the uniform variable
        glUniformMatrix4fv(transformLoc2, 1, GL_FALSE, glm::value_ptr(trans2)); // Set the uniform variable
