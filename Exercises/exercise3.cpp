...
// Source code from the fragment shader (orange color)
...
// Source code from the fragment shader (yellow color color)
...
int main() {
    ...
    fragmentShader[0] = glCreateShader(GL_FRAGMENT_SHADER); // Create a fragment shader object 1
    fragmentShader[1] = glCreateShader(GL_FRAGMENT_SHADER); // Create a fragment shader object 2
    glShaderSource(fragmentShader[0], 1, &fragmentShaderSourceOrange, NULL); // Attach the shader source code to the shader object
    glCompileShader(fragmentShader[0]); // Compile the shader
    glShaderSource(fragmentShader[1], 1, &fragmentShaderSourceYellow, NULL); // Attach the shader source code to the shader object
    glCompileShader(fragmentShader[1]); // Compile the shader



    // Create a shader program object to link the shaders created previously
    unsigned int shaderProgram[2];
    shaderProgram[0] = glCreateProgram(); // Final linked version of multiple shaders combined
    // Linking: Links the output of the vertex shader to the input of the fragment shader
    glAttachShader(shaderProgram[0], vertexShader);
    glAttachShader(shaderProgram[0], fragmentShader[0]);
    glLinkProgram(shaderProgram[0]); // Link the shaders
    ...
    // Bind the Second Shader Program
    shaderProgram[1] = glCreateProgram(); // Final linked version of multiple shaders combined
    glAttachShader(shaderProgram[1], vertexShader);
    glAttachShader(shaderProgram[1], fragmentShader[1]);
    glLinkProgram(shaderProgram[1]); // Link the shaders
    ...
    while (!glfwWindowShouldClose(window)) { // Check if the window should close
        ...
        glUseProgram(shaderProgram[0]); // Activate the shader program object
        ...
        glUseProgram(shaderProgram[1]); // Activate the shader program object
        glBindVertexArray(VAOs[1]); // Bind the vertex array object
        glDrawArrays(GL_TRIANGLES, 0, 3); // Draw the triangles
