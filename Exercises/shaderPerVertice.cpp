...
// Source code from the fragment shader with a color to each vertice
const char *fragmentShaderSourceVerticeColor= "#version 330 core\n"
    "out vec4 FragColor;\n"
    "in vec3 ourColor;\n" // Set this variable in the OpenGL code
    "void main()\n"
    "{\n"
...
int main() {
    ...
    Shader ourShader("shaderPerVertice.vs", "shaderPerVertice.fs");
    ...
    // Position attribute
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6*sizeof(float), (void*)0); // Stride value of 6 times the size of a float in bytes (= 24 bytes) 
    glEnableVertexAttribArray(0); // Enable the vertex attribute at location 0


    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6*sizeof(float), (void*)(3*sizeof(float))); // The color attribute starts after the position data so the offset is 3 * sizeof(float) in bytes (= 12 bytes). 
    glEnableVertexAttribArray(1); // Enable the vertex color attribute at location 1
